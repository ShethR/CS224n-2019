...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        H, (h_n, c_n) = self.encoder(X)
        H, _ = pad_packed_sequence(H)
        h_n = torch.cat((h_n[0,:,:], h_n[1,:,:]), dim=-1)
        c_n = torch.cat((c_n[0,:,:], c_n[1,:,:]), dim=-1)

        h_0_dec = self.h_projection(h_n)
        c_0_dec = self.c_projection(c_n)
        dec_init_state = (h_0_dec, c_0_dec)
        enc_hiddens = H.permute(1,0,2)
        ### END YOUR CODE
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ###         where tgt_len = maximu target sentence length, b = batch size, e = embedding size.
